...
//!
//! This example demonstrates using the `rust-ofrep-worker-forking` package
//! (which uses forking-flagd-evaluator) to create an OFREP-compliant feature
//! flag evaluation service on Cloudflare Workers.
...
async fn main(mut req: Request, _env: Env, _ctx: Context) -> Result<Response> {
    ...
    match (method, path.as_ref()) {
        // Health check
        (Method::Get, "/") => {
            cors_headers(Response::ok("flagd OFREP Rust Worker (forking) is running")?)
        }
